...
|                    |                                                                                                                                                                                                                       |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **What**           | A prepaid spend-cap / metering service: it decides whether an agent's (or customer's) wallet may spend an amount _before_ expensive work runs.                                                                        |
| **Why a service**  | A read-then-write counter overspends under concurrency — two callers read the same balance and both spend it. LedgerCap makes the decision _and_ records the movement as one atomic Aurora DSQL statement.            |
...
| **Money unit**     | Integer nanodollars internally (1 nano = 1e-9 USD). The API accepts `amountNanos` _or_ `amountCents`, never both. Floats are never used for money.                                                                    |
...
LedgerCap is a _reusable accounting boundary_. The consuming product keeps what
...
manage tokens. It calls the _same_ library functions as the data plane, so the two
...
protocol but with _optimistic_ concurrency: conflicting writes abort with SQLSTATE
...
A wallet is _resolved_ from an external identifier such as
...
| `holds`    | Reserved future spend                               | Direct charges spend _available_ (not total) balance, so they can't consume credit already promised to an in-flight operation.                                                      |
...
_prevented_, not reported after the fact.
...
The reservation immediately reduces _available_ balance, so concurrent calls
...
- **`40001`** — DSQL optimistic-concurrency conflict; retried. (`23505`,
  unique-violation, is _not_ retried.)
